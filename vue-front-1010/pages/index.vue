               ...
               <li v-for="course in courseList" :key="course.id">
                  <div class="cc-l-wrap">
                    <section class="course-img">
                      ...
                      />
                      <div class="cc-mask">
                        <a href="/course" title="开始学习" class="comm-btn c-btn-1"
                          >开始 学习</a
                        >
                      </div>
                    </section>
...
import index from "@/api/index"; // 引入添加热门课程以及讲师的操作
  ...
  methods: {
    ...
    getBannerList() {
      banner.getListBanner()
        .then(response => {
          // 两个data是因为在封装的时候并没有返回response.data，是直接将其返回的
    ...
    getHotCourseAndTeacher() {
      index.getIndexData().then((resp) => {
        this.courseList = resp.data.data.eduList;
